...
public struct RecordModal {
    var recordID: CKRecord.ID
    var href: String
    var isMet = false
}

public typealias SaveCompletion = (CKRecord?, Error?) -> Void
public typealias ValidateCompletion = (CKAccountStatus, Error?) -> Void
public typealias QueryCompletion = (CKQueryOperation.Cursor?, Error?) -> Void
public typealias FetchRecordCompletion = (CloudData) -> Void

public protocol CloudSaver {
    
}

public extension CloudSaver {
    /// 复制私有区域数据到公共区域
    ///
...
extension CKRecord {
    ...
    public func load(netDisk: CloudData) {
    ...
    public func load(record: CKRecord) {
    ...
    ///
    /// - Returns: 网盘数据模型
    public func convertModal() -> CloudData {
